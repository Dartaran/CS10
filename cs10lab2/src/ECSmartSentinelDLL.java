 ...
 */
public class ECSmartSentinelDLL<T> implements CS10LinkedList<T> {
	// Instance variables.
	private Element<T> currentPred; // element before current in list
	private Element<T> head; // head of list
	private Element<T> lastElement; // stores last element for addLast to be in O(1) time
	...
	public void add(T obj) {
		...
		// if the element after current (The element after after previous) is null
		// it must be the last element so update last element
		if (currentPred.next.next == null) lastElement = currentPred.next;
	}
	...
	public void remove() {
		...
		// if we are removing the last element then the last element is actually
		// the previous element of current, or just previous here
		if (currentPred.next.equals(lastElement)) lastElement = currentPred;
	...
	public void addLast(T obj) {
		if (isEmpty())
			...
			currentPred = lastElement;
			currentPred.next = new Element<T>(obj); // simply insert past last element
		}
	}
