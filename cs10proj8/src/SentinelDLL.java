 ...
 * SentinelDLL.java
 * 
 * Implementation of circular, doubly linked list with a sentinel WARNING: This
 * implementation is guaranteed to work only if always given objects that are
 * immutable (or at least do not change). Handling errors by throwing exceptions
 * would be a better way of dealing with errors, but this is a demo program.
 * 
 * @author THC
...
public class SentinelDLL<T> implements CS10LinkedList<T> {
	...
	/**
	 * Constructor for a circular, doubly linked list with a sentinel. Makes an
	 * empty list.
	 */
	public SentinelDLL() {
	...
	public boolean contains(T obj) {
		...
		// If we found it before getting back to the sentinel, it was in the
		// list.
	...
	/**
	 * @return true if there is a previous item (current item is not the first
	 *         item).
	 */
	...
	public void append(SentinelDLL<T> other) {
		other.sentinel.next.previous = sentinel.previous; // set this tail to other's head
		sentinel.previous.next = other.sentinel.next; // the next entry after this tail should be set to the header of other
	    sentinel.previous = other.sentinel.previous; // set the tail to the tail of the other list
	    other.clear(); // clear the other list (already implemented in constant time)
	}
}
