...
public class BinaryTree<E> {
	...
	public boolean isRoot() {
		return parent == null; // if no parent this must be the root
	}
	...
	/**
	 * @return the number of nodes (internal and leaf) in the subtree rooted at
	 *         this node.
	 */
	public int size() {
	 ...
	 */
	public int depth() {
		// first checks if this is the root, if so depth = 0
		// if not the root, then add 1 to depth and call the parent's
		// depth function to check if the parent is root. if the parent
		// is not root then adds 1 and repeats.
		return (parent == null ? 0 : 1 + parent.depth());
	}

	/**
	 * @return true if the subtrees rooted at this node and other have the same
	 *         structure and data, false otherwise
	 */
	public boolean equals(Object other) {
		if (other instanceof BinaryTree<?>) {
			@SuppressWarnings("unchecked")
			BinaryTree<E> t = (BinaryTree<E>) other;
			return (hasLeft() == t.hasLeft() && hasRight() == t.hasRight() && data.equals(t.data)
					&& (!hasLeft() || left.equals(t.left)) && (!hasRight() || right.equals(t.right)));
		} else
			return false;
	}

	/**
	 * @return an ArrayList of the data in the leaves of the subtree rooted at
	 *         this node, in order from left to right
	 */
	...
	/**
	 * Recursively construct a String representation of the subtree rooted at
	 * this node, starting with the given indentation and indenting further
	 * going down the tree.
	 * 
	 * @param indent
	 *            a String for indenting this node
	 * @return the String representation
	 */
	private String toStringHelper(String indent) {
		String ret = "";

		// If this node has children, indent them two more spaces than this
		// node.
		if (hasRight())
	...
	/**
	 * Create a List storing the the data values in the subtree rooted at this
	 * node, ordered according to the preorder traversal of the subtree.
	 * 
	 * @param dataList
	...
	/**
	 * Create a List storing the the data values in the subtree rooted at this
	 * node, ordered according to the inorder traversal of the subtree.
	 * 
	 * @param dataList
	 *            the list to be returned.
	 */
	...
	/**
	 * Create a List storing the the data values in the subtree rooted at this
	 * node, ordered according to the postorder traversal of the subtree.
	 * 
	 * @param dataList
	 *            the list to be returned.
	 */
	...
	/**
	 * Reconstruct a tree from preorder and inorder traversals, assuming that no
	...
	public static <E> BinaryTree<E> reconstructTree(List<E> preorder, List<E> inorder) {
		if (preorder.size() > 0) { // is this tree non-empty?
			...
			E rootValue = iterPre.next(); // the first value in the preorder
											// list is from the root
			E inValue = iterIn.next();

			// Find values in the left subtree and copy them into leftPre and
			// leftIn.
			while (!rootValue.equals(inValue)) {
				// Because the root appears before the left subtree, the
				// iteration through
				// the preorder list needs to be one spot ahead of the iteration
				// through
				// the inorder list.
			...
			// The iteration through the inorder list has caught up to the
			// preorder
			// list's iteration. In other words, the next values in each of
			// these
	...
	public int countLeaves() {
		// first checks if this node is a leaf. if so leaf count = 1
		// if this node isn't a leaf, counts the leaves in the left child
		// subrooted tree and the right child subrooted tree recursively
		return (isLeaf() ? 1 : ((hasLeft() ? left.countLeaves() : 0) + (hasRight() ? right.countLeaves() : 0)));
	}

	/**
	 * @return a shallow copy of this treef
	 */
	public BinaryTree<E> copy() {
		// simply instantiate a new tree using the
		// same data with a copy of the left and
		// right children
		return new BinaryTree<E>(data, left == null ? null : left.copy(), right == null ? null : right.copy());
	}
	 ...
	 * 
	 * @param other
	 *            the node you would like to be the parent
	 */
	public void setParent(BinaryTree<E> other) {
		parent = other;
	}
	...
	public static void main(String[] args) {
		BinaryTree<MutableInteger> tree = new BinaryTree<MutableInteger>(new MutableInteger(3),
				new BinaryTree<MutableInteger>(new MutableInteger(2),
						new BinaryTree<MutableInteger>(new MutableInteger(4)),
						new BinaryTree<MutableInteger>(new MutableInteger(1),
								new BinaryTree<MutableInteger>(new MutableInteger(6)), null)),
				new BinaryTree<MutableInteger>(new MutableInteger(7),
						new BinaryTree<MutableInteger>(new MutableInteger(5)), null));
