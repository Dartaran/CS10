...
public class MyHashMap implements MyMapADT {
	...
	public MyHashMap(int capacity) {
		...
		scale = rand.nextInt(prime - 1) + 1; // randomize scale
		shift = rand.nextInt(prime); // randomize shift
		createTable();
	}
	...
	public String toString() {
		...
		for (int i = 0; i < capacity; i++) { // iterate through table
			if (table[i] == null) // empty table skip
				continue;
			for (Entry e : table[i].getTable()) { // get each table entry
				if (e.getKey() == null)
					continue;
				// print the index and the keys in each index
				// multiple keys = collision
				debug += i + ": " + e.getKey() + " ";
			}
		}
